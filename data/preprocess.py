...
def save_dataset(
    ...
    train_props, test_props, train_smiles, test_smiles = train_test_split(
        properties, smiles_indices, test_size=test_size
    )
    ...
    logger.info(
        f"Saved preprocessing datasets and mappings in the directory: {output_dir}"
    )
...
def preprocess_data(
    ...
    properties = data[
        ["polararea", "complexity", "heavycnt", "hbonddonor", "hbondacc"]
    ].values
    ...
    char_to_idx = {
        char: idx + 3 for idx, char in enumerate(sorted(set("".join(smiles))))
    }
    char_to_idx["<pad>"] = 0
    char_to_idx["<start>"] = 1
    char_to_idx["<end>"] = 2
    ...
    smiles_indices = [
        [char_to_idx["<start>"]]
        + [char_to_idx[char] for char in smi]
        + [char_to_idx["<end>"]]
        + [char_to_idx["<pad>"]] * (max_smiles_len - len(smi) - 2)
        for smi in smiles
    ]
    logger.info(
        "Data processing: conversion of SMILEs notation of molecules to sequence complete"
    )
