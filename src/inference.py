...
def preprocess_properties(csv_file: str, scaler_path: str) -> np.ndarray:
...
def run_inference(input_csv: str, output_csv: str):
    ...
    mappings_path = os.path.join(
        config.data.processed_dataset_path, "char_mappings.pkl"
    )
    ...
    for i in range(properties.shape[0]):
        prop_tensor = (
            torch.tensor(properties[i], dtype=torch.float32).unsqueeze(0).to(device)
        )
        ...
        smiles = "".join(
            [
                ...
                if idx in idx_to_char and idx != 0
            ]
        )
